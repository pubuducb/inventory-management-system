...
Instead of using Gin’s default binding, the code uses `json.Decoder` with `DisallowUnknownFields()`. This ensures that any unexpected / malicious fields in the request body are rejected, improving API security and preventing silent bugs from malformed input.
